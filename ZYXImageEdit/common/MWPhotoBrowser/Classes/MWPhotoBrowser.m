...
- (void)viewDidLoad {
    ...
    }else{//如果_actionButton没有被使用，而又需要删除按钮时  张永旭
        if(self.displayDeleteActionButton){
             _actionButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemTrash target:self action:@selector(actionDeleteButtonPressed:)];
        }
    }
...
- (void)actionDeleteButtonPressed:(id)sender{
    NSLog(@"_currentPageIndex is %ld",_currentPageIndex);
    if([self.delegate respondsToSelector:@selector(photoBrowser:actionDeleteButtonPressedForPhotoAtIndex:)]){
        [self.delegate photoBrowser:self actionDeleteButtonPressedForPhotoAtIndex:_currentPageIndex];
    }
