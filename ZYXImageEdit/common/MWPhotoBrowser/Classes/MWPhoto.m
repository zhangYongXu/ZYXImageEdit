...
- (id)initWithFilePath:(NSString *)path {
	if ((self = [super init])) {
		_photo = [NSURL fileURLWithPath:path];
	}
	return self;
}
...
- (void)performLoadUnderlyingImageAndNotify {
    ...
    if (_image) {
        ...
        if ([[[_photo scheme] lowercaseString] isEqualToString:@"assets-library"]) {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                @autoreleasepool {
                    @try {
                        ...
                        [assetslibrary assetForURL:self->_photo
        ...
        } else if ([_photo isFileReferenceURL]) {
            
            // Load from local file async
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                @autoreleasepool {
                    @try {
                        self.underlyingImage = [UIImage imageWithContentsOfFile:self->_photo.path];
                        if (!self->_underlyingImage) {
                            MWLog(@"Error loading photo from path: %@", _photo.path);
                        }
                    } @finally {
                        [self performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
                    }
            ...
            @try {
                ...
                _webImageOperation = [manager downloadImageWithURL:_photo
                                                           options:0
